...
test("renders the compact composer with the standard Send button and no session controls", () => {
  ...
  // Local behavior (kept over upstream): the compact quote-popup composer still
  // supports image attachments — the hidden file input stays mounted, only the
  // session controls are stripped.
  assert.match(html, /type="file"/);
  assert.doesNotMatch(html, /Attach image|Change tool preset/);
});
