...
function parseLaunchOptions(args = process.argv.slice(2), env = process.env, { cwd = process.cwd() } = {}) {
  ...
  return {
    help: false,
    port: normalizePort(values.port ?? resolvedEnv.PORT ?? "30141"),
    hostname: values.hostname ?? resolvedEnv.PI_WEB_HOSTNAME ?? "127.0.0.1",
    openBrowser: !values["no-open"] && !isEnabled(resolvedEnv.PI_WEB_NO_OPEN),
  };
}

module.exports = { parseLaunchOptions, readProjectEnv, getHelpText };
