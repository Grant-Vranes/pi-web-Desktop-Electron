...
Чтобы задать постоянное значение для проекта, скопируйте шаблон и измените `PORT`:

```bash
cp .env.example .env.local
# Отредактируйте .env.local, например: PORT=8080
```

`.env.local` игнорируется Git и используется командами `npm run dev`, `npm run desktop:dev`, `npm run start` и `pi-web`, запущенными из этого каталога. Параметры командной строки и уже экспортированные переменные окружения имеют приоритет.
...
Сервер разработки по умолчанию доступен по адресу [http://127.0.0.1:30141](http://127.0.0.1:30141), а при настройке использует `PORT` из `.env.local`. Основные проверки запускаются командами:
...
Репозиторий включает Electron-обертку в `desktop/`, которая запускает Pi Web локально и добавляет поведение в трее/фоновом режиме, а также интеграцию с нативным контекстным меню строки сессии.

### Отладочный режим десктопа

```bash
npm run desktop:dev
```

Запускает веб-сервер разработки и Electron одновременно. Electron подключается к `127.0.0.1:30141` и в режиме разработки переиспользует этот внешний сервер.

### Запуск десктоп-приложения на production-ассетах
...
`desktop:start` запускает Electron и стартует встроенный `pi-web` из `bin/pi-web.js` (или переиспользует уже запущенный сервер на том же порту).
...
Выполняет `npm run build` и упаковывает установщики через `electron-builder`.
Целевые форматы по умолчанию:

- macOS: `dmg`
- Windows: `nsis`
- Linux: `AppImage`

Артефакты сборки записываются в `dist/`.

> **Совет для Linux:** шаг `fpm` в `electron-builder` копирует распакованное приложение в системный временный каталог перед сжатием в `.deb`. Если `/tmp` — небольшой tmpfs (типично для Linux), копирование может завершиться ошибкой `Disk quota exceeded (Errno::EDQUOT)`. Перенаправьте временный каталог на путь на диске, чтобы этого избежать:
>
> ```bash
> TMPDIR=$(pwd)/.build-tmp npm run desktop:dist
> ```
>
> `.build-tmp/` уже добавлен в `.gitignore`.
