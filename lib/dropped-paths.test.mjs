...
test("buildDropPayload falls back to the legacy File#path for an external file", async () => {
  const { buildDropPayload } = await loadSubject();
  // No piDesktop: the runtime still exposes the absolute path directly on the
  // File (legacy Electron/webkit), so an external file yields an absolute-path
  // @mention instead of being dropped into the folder upload fallback.
  withWindow({}, () => {
    const payload = buildDropPayload({
      files: [{ name: "report.md", type: "", path: "/work/report.md" }],
      items: [{ webkitGetAsEntry: () => ({ isFile: true }) }],
      getData: () => "",
    });

    assert.equal(payload.pathMentions, '@"/work/report.md" ');
    assert.equal(payload.hasUnresolvedDirectory, false);
    assert.equal(payload.hasNonImageFiles, true);
  });
});
