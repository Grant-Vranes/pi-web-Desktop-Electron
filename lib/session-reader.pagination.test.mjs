...
test("buildSessionContext turnAnchors cover turns beyond the tail window", () => {
  const entries = linearChain(300);
  const ctx = buildSessionContext(entries, "e299", { tail: 50 });
  // Window holds 50 entries: e250..e299, of which the even ids are user turns.
  assert.equal(ctx.messages.length, 50);
  // Anchors span the whole branch: all 150 user turns, oldest first.
  assert.equal(ctx.turnAnchors.length, 150);
  assert.equal(ctx.turnAnchors[0].entryId, "e0");
  assert.equal(ctx.turnAnchors[0].kind, "user");
  assert.equal(ctx.turnAnchors[0].preview, "msg 0");
  assert.equal(ctx.turnAnchors.at(-1).entryId, "e298");
  // Every anchor beyond the window has no loaded message — placeholders.
  const loadedIds = new Set(ctx.entryIds);
  const outside = ctx.turnAnchors.filter((a) => !loadedIds.has(a.entryId));
  // Of the 150 anchors, 25 fall inside the loaded window → 125 placeholders.
  assert.equal(outside.length, 125);
  assert.ok(outside.every((a) => a.kind === "user" && typeof a.preview === "string"));
});

test("turnAnchors include compaction summaries and skip non-anchored entries", () => {
  const entries = [
    { id: "u0", parentId: null, type: "message", timestamp: new Date(1000).toISOString(), message: { role: "user", content: "hello world" } },
    { id: "a0", parentId: "u0", type: "message", timestamp: new Date(2000).toISOString(), message: { role: "assistant", content: [{ type: "text", text: "hi" }] } },
    { id: "c0", parentId: "a0", type: "compaction", timestamp: new Date(3000).toISOString(), summary: "summary text", firstKeptEntryId: "u0", tokensBefore: 10 },
    { id: "u1", parentId: "c0", type: "message", timestamp: new Date(4000).toISOString(), message: { role: "user", content: [{ type: "text", text: "next" }, { type: "image", data: "x", mimeType: "image/png" }] } },
  ];
  const ctx = buildSessionContext(entries, "u1", { tail: 1 });
  assert.deepEqual(ctx.turnAnchors.map((a) => [a.entryId, a.kind, a.preview]), [
    ["u0", "user", "hello world"],
    ["c0", "compaction", "summary text"],
    ["u1", "user", "next"],
  ]);
});
